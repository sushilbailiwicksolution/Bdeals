		...
		class My_Upload extends CI_Upload {
			 ...
			 */
				public function do_multi_upload($field){
					...
					log_message('debug', 'FILE OBJECT ['.print_r($_FILES, true).']');
					if(!isset($_FILES[$field])){
						log_message('debug', 'field not set ['.$field.']');
						return false;
					}
					...
					if(!$this->validate_upload_path()){
						...
						log_message('debug','Not a valid upload path');
						return FALSE;
					}
					...
					foreach ($_FILES[$field]["name"] as $i => $v) {
						...
						if(!is_uploaded_file($_FILES[$field]["tmp_name"][$i])){
							...
							log_message('debug', 'Error due to ['.$error.'] ['.$v.']');
							//Set error.
							switch($error){
								//UPLOAD_ERR_INI_SIZE
								case 1:

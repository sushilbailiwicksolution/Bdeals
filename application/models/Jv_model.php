...
class Jv_model extends CI_Model{
	...
	public function registerBusinessUser( $actType="NEW" ){
		...
		$userData = array('business_type'=>'jv_business','name'=>$name,'email'=>$this->session->userdata('userid'),'contact'=>$contact_number,'address'=>$address,'location'=>$location,'company_name'=>$company_name,'other_state'=>$other_state,'other_city'=>$other_city,'business_add_date'=>date('Y-m-d h:i:s'));
		...
		if( strcasecmp( $actType, "NEW" ) == 0 ){
			...
			if ( !$result && $this->db->error()['code'] == 1062 ){
				...
				if ( $this->db->affected_rows() > 0 ){
					...
					$userData = array('form_id'=>$this->session->userdata('last_form_id'),'added_date'=>date('Y-m-d h:i:s'));
	...
	public function getBusinessUserProfile( $businessType ){
		...
		$this->db->where('CUSTOMER_ID',$this->session->userdata('userid'));
		$this->db->where('BUSINESS_TYPE','jv_business');
		$this->db->order_by('form_id',"DESC");
		$this->db->limit(1);

		$userBusinessData = $this->db->get();
		log_message('debug', 'Jv_model: getBusinessUserProfile: ['.$this->db->last_query().']');
		if( !$userBusinessData ){
			return null;
		}else{
	...
	public function registerBusinessBasicInformation(){
		...
		$userData = array('key_headline'=>$keyHeadline,'description'=>$businessDesc,'location_country'=>$businessCountry,'location_region'=>$businessRegion,'location_state'=>$businessState,'other_state'=>$other_state,'other_city'=>$other_city,'location_city'=>$businessCity,'listing_category'=>$businessCategory,'listing_sub_category'=>$businessSubCategory,'listing_category_other'=>$listing_category_other,'listing_sub_category_other'=>$listing_sub_category_other,'legal_entity'=>$legalEntity,'yoe'=>$establishmentYear,'current_status_of_business'=>$currentBusinessStatus,'bstatus_other'=>$bstatus_other);
		...
		log_message('debug', 'Jv_model: registerBusinessBasicInformation: ['.$this->db->last_query().']');
		if($result ){
	...
	public function registerSellerBusinessDescription( $actType = "NEW" ){
		...
		'latest_revenu_currency'=>$latest_revenu_currency, 'latest_revenu'=>$latest_revenu,'latest_revenu_unit'=>$latest_revenu_unit, 'product_service_desc'=>$productServiceDesc, 'competition_market_desc'=>$competitionMarketDesc, 'growth_potential'=>$growthPotential, 'reason_for_sale'=>$reasonForSale);

		if( strcasecmp( $actType, "NEW" ) == 0 ){
			$userData['form_id'] = $this->session->userdata('last_form_id');
			$result = $this->db->insert( TBL_PREFIX.TBL_JV_BUSINESS_DETAILS, $userData );
			log_message('debug', 'Jv_model: registerSellerBusinessDescription: ['.$this->db->last_query().']');
			if ( !$result && $this->db->error()['code'] == 1062 ){
				return "duplicate";
			}else{
				if ($result){
	...
	public function getSellerBusinessDescription(){
		...
		log_message('debug', 'Jv_model: getSellerBusinessDescription: ['.$this->db->last_query().']');
	...
	public function registerSellerAdditionalInformation(){
		...
		for( $i=0; $i < $count; $i++){
			...
			$dataArr[$i]['real_state_value_unit']      = $this->input->post('ask_price_unit_real_'.$val);
			$dataArr[$i]['real_state_value']      = $this->input->post('real_state_value_'.$val);
			$dataArr[$i]['real_state_value_type']      = $this->input->post('ask_price_currency_real_'.$val);
		...
		for($j=0; $j < $count; $j++){
			...
			if(($dataArr[$j]['property_id']==NULL)){
				...
				$userData = array('form_id'=>$this->session->userdata('last_form_id'),'property_type'=>$dataArr[$j]['property_type'],'real_state_value_unit'=>$dataArr[$j]['real_state_value_unit'],'ask_price_real'=>$dataArr[$j]['ask_price_real'],'real_state_value'=>$dataArr[$j]['real_state_value'],'real_state_value_type'=>$dataArr[$j]['real_state_value_type'], 'facility_desc'=>$dataArr[$j]['facility_desc'], 'industrial_commercial'=>$dataArr[$j]['industrial_commercial'], 'total_land_area_type'=>$dataArr[$j]['total_land_area_type'], 'total_land_area'=>$dataArr[$j]['total_land_area'], 'built_up_area_type'=>$dataArr[$j]['built_up_area_type'], 'built_up_area'=>$dataArr[$j]['built_up_area'], 'open_area_type'=>$dataArr[$j]['open_area_type'], 'open_area'=>$dataArr[$j]['open_area'], 'other_area'=>$dataArr[$j]['other_area'],'carpet_area'=>$dataArr[$j]['carpet_area'],'added_date'=>date('Y-m-d h:i:s'));

...
class Startup_model extends CI_Model{
	...
	public function registerBusinessDealDetails( $actType = "NEW" ){
		...
		$userData = array('purpose_investment'=>$purpose_investment, 'investment_required_as'=>$investment_required_as, 'invester_role'=>$invester_role, 'propose_deal_structure'=>$propose_deal_structure);
...
public function registerSellerPackageAdmin(){
	...
	public function registerAdditionService(){
		...
		$userData = array('customer_id'=>$this->session->userdata('userid'), 'form_id'=>$form_id, 'feature_listing'=>$feature_listing, 'memorandum'=>$information_memorandum, 'plan'=>$business_plan, 'incubation'=>$incubation, 'startup-mentoring'=>$mentoring, 'fund_raising_advisory'=>$fund_raising_advisory, 'account_manager'=>$account_manager );
		$whereClause = array( 'form_id'=>$form_id, 'customer_id' => $this->session->userdata('userid') );

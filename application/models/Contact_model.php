...
class Contact_model extends CI_Model{
	...
	public function insertContactDetail(){
		...
		$data = array( "name" => $name,"email" => $email,"city" => $city,"address" =>$address,"contact_number" => $mobile,"query_purpose" =>$purposepurpose,"message" => $message,"query_datetime" =>date('Y-m-d h:i:s'),"mtime" => $timeval,"mdate" => $dateval,"mplace" => $place,"mtax_type" => $tax,"mtype" => $_POST['type'],"meeting_agenda" => $meeting_agenda,"meeting_venue" => $meeting_venue);
		$retVal = $this->db->insert( TBL_PREFIX.TBL_CONTACT_US_DETAIL, $data );
		log_message('debug', 'Contact_model: insertContactDetail: ['.$this->db->last_query().']');
		return $retVal;
	}
	...
	public function registerBusinessBasicInformation(){
		...
		log_message('debug', 'User_model: registerBusinessBasicInformation: ['.$this->db->last_query().']');
		if( !$result ){
			return "error";
		}else{
			if( $this->db->affected_rows() > 0 ){

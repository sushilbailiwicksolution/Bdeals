...
public function getBusinessStatusByFormId( $formId ){
	...
	public function registerBusinessUser( $actType="NEW" ){
		...
		$userData = array('business_type'=>'re_business');
		if( strcasecmp( $actType, "NEW" ) == 0 ){
			...
			if ( !$result && $this->db->error()['code'] == 1062 ){
				...
				if ( $this->db->affected_rows() > 0 ){
					...
					$result = $this->db->insert(TBL_PREFIX.TBL_REALESTATE_BUSINESS_DETAILS, $userData);
					log_message('debug', 'RE_model: registerBusinessUser: ['.$this->db->last_query().']');
	...
	public function getBusinessUserProfile( $businessType ){
			...
			$this->db->where('business_type',$businessType);
			
		$reapproved_count = $this->input->get("reapproved_count");
		$repending_count = $this->input->get("repending_count");
		$rerejected_count = $this->input->get("rerejected_count");
		$reinactive_count = $this->input->get("reinactive_count");
		$reclosed_count = $this->input->get("reclosed_count");
		$reall_count = $this->input->get("reall_count");
		...
		if($reapproved_count!='' and $repending_count=='' and $rerejected_count=='' and $reinactive_count=='' and $reclosed_count=='' and $reall_count=='' and $sort_status=='' and $fromDate=='' and $toDate=='' and $sort_city=='' and $category=='' and $pricetype==''){
			$this->db->where(array('status'=>$reapproved_count));
		}
		if($repending_count!='' and $reapproved_count=='' and $rerejected_count=='' and $reinactive_count=='' and $reclosed_count=='' and $reall_count=='' and $sort_status=='' and $fromDate=='' and $toDate=='' and $sort_city=='' and $category=='' and $pricetype==''){
			$this->db->where(array('status'=>$repending_count));
		}
		
		if($rerejected_count!='' and $reapproved_count=='' and $repending_count=='' and $reinactive_count=='' and $reclosed_count=='' and $reall_count=='' and $sort_status=='' and $fromDate=='' and $toDate=='' and $sort_city=='' and $category=='' and $pricetype==''){
			$this->db->where(array('status'=>$rerejected_count));
		}
		
		if($reinactive_count!='' and $reapproved_count=='' and $repending_count=='' and $rerejected_count=='' and $reclosed_count=='' and $reall_count=='' and $sort_status=='' and $fromDate=='' and $toDate=='' and $sort_city=='' and $category=='' and $pricetype==''){
			$this->db->where(array('status'=>$reinactive_count));
		}
		
		if($reclosed_count!='' and $reapproved_count=='' and $repending_count=='' and $rerejected_count=='' and $reinactive_count=='' and $reall_count=='' and $sort_status=='' and $fromDate=='' and $toDate=='' and $sort_city=='' and $category=='' and $pricetype==''){
			$this->db->where(array('status'=>$reclosed_count));
		}
		
		if($reall_count!='' and $reclosed_count=='' and $reapproved_count=='' and $repending_count=='' and $rerejected_count=='' and $reinactive_count=='' and $sort_status=='' and $fromDate=='' and $toDate=='' and $sort_city=='' and $category=='' and $pricetype==''){
...
BETWEEN DATE_FORMAT("'.$fromDate.'","%Y-%m-%d") AND DATE_FORMAT("'.$toDate.'","%Y-%m-%d")');
		...
		log_message('debug', 'Last Query RE ['.$this->db->last_query().']');
		if( !$userBusinessData ){
			return null;
		}else{
	...
	public function getPropertyType( $businessType ){
			$this->db->select(TBL_PREFIX.TBL_REALESTATE_BUSINESS_DETAILS.'.id,'.TBL_PREFIX.TBL_REALESTATE_BUSINESS_DETAILS.'.user_id,'.TBL_PREFIX.TBL_REALESTATE_BUSINESS_DETAILS.'.status');
		
			$this->db->from(TBL_PREFIX.TBL_BUSINESS_DETAILS);
			$this->db->join(TBL_PREFIX.TBL_REALESTATE_BUSINESS_DETAILS, TBL_PREFIX.TBL_BUSINESS_DETAILS.'.form_id = '.TBL_PREFIX.TBL_REALESTATE_BUSINESS_DETAILS.'.form_id','left');
			
			$this->db->where('form_id',$businessType);
		
		$sellBusinessBasicInformation = $this->db->get();
		log_message('debug', 'Last Query getPropertyType ['.$this->db->last_query().']');
		if( !$sellBusinessBasicInformation ){
			return null;
		}else{
			$dataToReturn = $sellBusinessBasicInformation->result_array();
			return $dataToReturn;
		}
	...
	public function registerBusinessBasicInformation(){
		...
		$userData = array('key_headline'=>$keyHeadline,'description'=>$businessDesc,'location_country'=>$businessCountry,'location_region'=>$businessRegion,'location_state'=>$businessState,'location_city'=>$businessCity,'property_type'=>$property_type,'saleable_area'=>$saleable_area,'age_construction'=>$age_construction,'authority_approval'=>$authority_approval);
	...
	public function registerSellerBusinessDetails( $actType = "NEW" ){
		...
		$lease_period       = $this->input->post('location_advantages');
	...
	public function getUserFavourite(){
		...
		public function updateRealestate($formid){
	...
	$other_state           = $this->input->post('other_state');
		...
		$userData1 = array('key_headline'=>$keyHeadline,'description'=>$bus_desc,'location_country'=>$country,'location_region'=>$businessRegion,'location_state'=>$businessState,'location_city'=>$city,'property_type_old'=>$property_type,'saleable_area'=>$saleable_area,'age_construction'=>$age_construction,'authority_approval'=>$authority_approval,'other_city'=>$other_city,'other_state'=>$other_state);
		

		$userData2 = array('tenancy_details'=>$tenancy_details, 'lease_period'=>$lease_period, 'lease_start'=>$lease_start,
		'lease_end'=>$lease_end, 'lock_period'=>$lock_period, 'security_received'=>$security_received, 'monthly_rental'=>$monthly_rental,
		'annual_rental'=>$annual_rental,'annual_maintenance'=>$annual_maintenance, 'escalation_after'=>$escalation_after, 
		'escalation_percent'=>$escalation_percent,'features'=>$features,'location_advantages'=>$location_advantages,'reason_sale'=>$reason_sale,'roi_present'=>$roi_present,'roi_escalation'=>$roi_escalation,'other_income'=>$other_income,'price_currency'=>$price_currency,'price_value'=>$price_value,'price_unit'=>$price_unit);
		
		$this->db->update( TBL_PREFIX.TBL_BUSINESS_DETAILS, $userData1, array( 'form_id' => $formid ) );
		
		log_message('debug', 'RE_model: update realestate data 2222  ['.$this->db->last_query().']');
		
		$result = $this->db->update( TBL_PREFIX.TBL_REALESTATE_BUSINESS_DETAILS, $userData2, array( 'form_id' => $formid) );
		log_message('debug', 'RE_model: update realestate data  ['.$this->db->last_query().']');
		if( !$result ){
			return "error";
		}else{
			if( $this->db->affected_rows() > 0 ){

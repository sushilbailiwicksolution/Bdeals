...
class Jointventures extends CI_Controller {
	...
	public function userProfileCreate(){
		...
		if ( strcasecmp( $frmValue, 'sb_up' ) == 0 ){
			...
			if( $dbResult ){
				...
				if( $this->input->post('frm_id') != null ){
					$dbResult = $this->JV->registerBusinessBasicInformation("UPDATE", $this->session->userdata('last_form_id'));
				}else{
					$dbResult = $this->JV->registerBusinessUser("NEW")[0];
					$dbResult = $this->JV->registerBusinessBasicInformation("UPDATE", $this->session->userdata('last_form_id'));
				}
				...
				if( $dbResult ){
					...
					redirect('list-your-business-description-jv');
				}else{
					redirect('list-your-business-basic-information-jv'); //Redirect user on business description page in case any db error
				}
			...
			if($this->session->userdata('last_form_id')=='NEW'){
				...
				if( $this->input->post('frm_id') != null ){
					$dbResult = $this->JV->registerSellerBusinessDescription("UPDATE");
				}else{
					$dbResult = $this->JV->registerSellerBusinessDescription( "NEW" );
				}

				if( $dbResult ){
					redirect('list-your-business-additional-details-jv');
				}else{
					redirect('list-your-business-description-jv'); //Redirect user on business description page in case any db error
				}
			}
			...
			if($this->session->userdata('last_form_id')=='NEW'){
				...
				if( $this->JV->registerAdditionService() ){
					/**********************mail sent after form submission start here for jv business*****/
...
if($this->email->send()){
         ...
         //  echo "email not sent";
					...
					/**********************mail sent after form submission end here for jv business*****/
			...
			if( $this->JV->saveAllDetails( $fileInfoToSaveInDB ) ){
				if( strcasecmp( $frmValue, 'UA_FU' ) == 0 ){
					//$next_tab = $this->input->post('next_tab');
					//return array('status'=>'true','next_tab'=>$next_tab);
					redirect('login-business-profile-form-jv/Tanuj/'.$this->input->post('frm_id').'/1234/abc#Packagedetails');
				}else{
					redirect('list-your-business-additional-details-uploads-jv'); //Redirect user after successfull business registration
				}
			}else{
				if( strcasecmp( $frmValue, 'UA_FU' ) == 0 ){
					//$next_tab = $this->input->post('next_tab');
	...
	public function basicInformation(){
		...
		if($this->session->userdata('last_form_id')=='NEW'){
			...
			$this->load->model('Jv_model', 'JV');
			$data['sellBusinessBasicInformation'] = $this->JV->getBusinessUserProfile('joint_ventures');
		}
		$this->load->view('include/header', $header );
		$this->load->view('joint_ventures/business_as_jv/basic-information', $data);
		$this->parser->parse('include/footer', $footer );
	}
	...
	public function businessDescription(){
		...
		if($this->session->userdata('last_form_id')=='NEW'){
			...
			$data['sellBusinessDescription'] = $this->JV->getSellerBusinessDescription('joint_ventures');
		}

		$this->load->view('include/header', $header );
		$this->load->view('joint_ventures/business_as_jv/business-description', $data);
		$this->parser->parse('include/footer', $footer );
	}
	...
	public function benefitsJv(){
		...
		$this->load->view('joint_ventures/benefits-jv');
			...
			public function JVBusinessPreview(){
                ...
                $this->load->view('include/header', $header );
                $this->load->view('joint_ventures/business_as_jv/jv-business-preview-form', $data);
                $this->parser->parse('include/footer', $footer );
	}
}
?>
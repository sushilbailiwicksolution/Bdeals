...
class Startup extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->model('admin/Startup_model', 'startup');
		$data['userList'] = $this->startup->getBusinessUserProfile('startup_business');
	...
	public function startup_page(){
		...
		$this->load->model('admin/Startup_model', 'startup');
		$userList = $this->startup->getBusinessUserProfile('startup_business');
		...
		foreach($userList as $r){
			...
			$assignToDetails = $this->startup->getAssignToDetails( $this->user->getCustomerDetailsByCustId($r['customer_id'], array('id', 'name'))[0]['id'] );
			
			if ( strcasecmp($this->session->userdata('admin_type'), 'admin' ) == 0 ){
				$businessStatus  = $this->startup->getBusinessStatusByFormId( $r['form_id'] );
				...
				if ( strcasecmp( $this->session->userdata('admin_userid'), $assignToDetails[0]['user_id'] ) == 0 ){
					...
					$data[] = array($i,$dateval,$r['form_id'],$this->cachemethods->getCatNameByCId($r['listing_category']),$r['name'],$r['contact'], $r['email'], $city,$r['expiry_date'],'<a data-toggle="modal" data-target="#myModal2" href="'.$r['form_id'].'" class="contactList" role="button">'.$r['num_business_viewed'].'</a>','<a data-toggle="modal" data-target="#myModal" href="'.$r['form_id'].'" class="favelist" role="button">'.$r['add_to_favorite'].'</a>','<a data-toggle="modal" data-target="#myModal3" href="'.$r['form_id'].'" class="SellerCount" role="button">'.$SellerCount.'</a>', $assignToDetails[0]['user_id'], $businessStatus, '<a href="'.base_url().'admin/startup_explore/'.$assignToDetails[0]['id'].'/'.$r['form_id'].'/'.$r['customer_id'].'/abc" class="btn btn-sm btn-danger" role="button">Explore</a>');
					$i++;
				}
			}
		}
	...
	public function startup_explore( $assignedTo=NULL, $selectedFormId, $customerId ){
				$this->load->library('parser');
                $this->load->helper('html');
                $header['title']  = 'Business Deals';
		...
		$data['sellBusinessBasicInformation'] = $this->startup->getSellerBusinessPitch($selectedFormId);
		...
		$data['customerId'] = $customerId;
                ...
                $this->load->view('admin/include/header', $header );
				$this->load->view('admin/startup_explore', $data);
                $this->parser->parse('admin/include/footer', $footer );
	}
	...
	public function UpdateStartupForm(){
					...
					$dbResult2 = $this->startup->registerBusinessPitch("UPDATE");
	...
	public function UpdateAdminStartupForm(){
...
/**************update seller*****************/
	...
	public function getApprovedCount() //for all approved startup form
		{
			
				$this->load->model('admin/Jv_Business_model','business');
				$data['queryCount']  = $this->business->getApprovedCount('startup_business');
				echo $data['queryCount'];
			
		}
	public function getAllCount() //for all startup form
		{
			
				$this->load->model('admin/Jv_Business_model','business');
				$data['queryCount']  = $this->business->getAllCount('startup_business');
				echo $data['queryCount'];
			
		}	

	public function getPendingCount() //for all pending startup form
		{
			
				$this->load->model('admin/Jv_Business_model','business');
				$data['queryCount']  = $this->business->getPendingCount('startup_business');
				echo $data['queryCount'];
			
		}
public function getRejectedCount() //for all pending startup form
		{
			
				$this->load->model('admin/Jv_Business_model','business');
				$data['queryCount']  = $this->business->getRejectedCount('startup_business');
				echo $data['queryCount'];
			
		}		
public function getInactiveCount() //for all inactive startup form
		{
			
				$this->load->model('admin/Jv_Business_model','business');
				$data['queryCount']  = $this->business->getInactiveCount('startup_business');
				echo $data['queryCount'];
			
		}
public function getClosedCount() //for all closed startup form
		{
			
				$this->load->model('admin/Jv_Business_model','business');
				$data['queryCount']  = $this->business->getClosedCount('startup_business');
				echo $data['queryCount'];
			
		}
/***************download report in xls format for users start here*******/
 public function StartUpReport()
 ...
 $this->load->model('admin/Seller_model', 'seller');
  $this->load->model('admin/Startup_model', 'startup');
  $data['userBusinessData']  = $this->startup->getStartUpReport();
  
 //print_r($data['userBusinessData']);
  
// die;
  
  $filename = "Startup-data-" . date('d-m-Y') . ".xls";



  
  
  $this->load->view('admin/startup_data_report', $data);
